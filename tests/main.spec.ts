...
describe('Resolving example', () => {
    ...
    // eslint-disable-next-line max-lines-per-function
    describe('Fill', () => {
        ;(it('Lightning: should swap Ethereum USDC -> LN BTC. Single fill only', async () => {
            try {
            ...
            it('Lightning: should swap Ethereum LN BTC -> USDC. Single fill only', async () => {
                try {
                    await bob_rpc.connect()
                } catch (error) {
                    console.error('❌ Error:', error.message)
                ...
                try {
                    ...
                    // Create invoice
                    const invoice = await bob_rpc.invoiceWithPreimage(
                        100000, // 100k msat (0.001 sat)
                        `1inch-${Date.now()}`,
                        '1inch swap invoice with preimage',
                        secret.slice(2)
                ...
                try {
                    await alice_rpc.connect()
                    const status = await alice_rpc.pay(bolt11)
            ...
            it('TVM: should swap Ethereum USDT -> Tip3 USDT. Single fill only', async () => {
            ...
            it('TVM: should swap Tip3 USDT -> Ethereum USDT. Single fill only', async () => {
                try {
                    await alice_rpc.connect()
                } catch (error) {
                    console.error('❌ Error:', error.message)
