...
describe('Resolving example', () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            writeFileSync('.redeemWIF', refundKeyPair.toWIF())
        }
    ...
    async function createEscrowAddress(hashLock: string): Promise<{address: string; wshOutput: any; timeLock: number}> {
        ...
        const wshDescriptor = `wsh(${miniscript
            .replace(
                '@redeemKey',
                redeemKeyPair.publicKey.toString('hex')
            )
    ...
    async function broadcastSpendingTransaction(
        ...
        new Output({
            descriptor: `addr(tb1qgx8hj9juuka6fse4ewl4qqq6m96hv5993c3wka)`,
            network
    ...
    describe('Fill', () => {
        it('should swap Ethereum USDC -> Onchain BTC. Single fill only', async () => {
            ...
            const shalock = '143250e48d33d9e780af5f7f89bd329088a03240568cb03fef87449528ec8226' //crypto.createHash('sha256').update(sbytes).digest('hex')
            console.log(secret)
            console.log(shalock)
            ...
            if (hasFunds) {
                console.log(`\n✅ FUNDED! Found ${utxo.length} UTXO(s)`)
            }

            await increaseTime(11)

            //const result = await broadcastSpendingTransaction(utxo, secret.slice(2), shalock)
